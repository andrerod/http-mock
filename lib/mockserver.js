...
driverFactory.modes.neutral = function (proxy, session) {
  ...
  driver.call = function (request, response) {
    ...
    var processResponse = function (proxyResponse) {
      var requestStop = new Date();

      writeLog("    STATUS: " + proxyResponse.statusCode + ' ' + (requestStop.getTime() - requestStart.getTime()) + 'ms');

      // Headers need to be written before everything else
      if (request.method === 'HEAD') {
        proxyResponse.headers['Connection'] = 'close';
      }
...
driverFactory.modes.recording = function (proxy, session) {
  ...
  driver.close = function () {
    ...
    for (var i in records) {
      ...
      records[i].requestUrl = normalizeUrl(records[i].requestUrl);
    }
...
function normalizeUrl (requestUrl) {
  return url.format(url.parse(requestUrl));
}